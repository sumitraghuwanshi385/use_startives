...
import { IdeaStarIcon, STARTUP_CATEGORIES, BUSINESS_MODELS, WORK_MODES, MapPinIconHero, COUNTRIES, IdentificationIcon, ChevronLeftIcon, PhotoIcon, AppContextLinkIcon as LinkIcon, XMarkIcon } from '../constants';
...
const Bars3BottomLeftIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className || "w-6 h-6"}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12" /></svg>
);
...
const BookmarkSquareIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className || "w-6 h-6"}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 3.75V16.5L12 14.25L7.5 16.5V3.75m9 0H18A2.25 2.25 0 0120.25 6v12A2.25 2.25 0 0118 20.25H6A2.25 2.25 0 013.75 18V6A2.25 2.25 0 016 3.75h1.5m9 0h-9" /></svg>
);
        ...
        <div className="absolute z-50 w-full mt-1.5 bg-[var(--component-background)] border border-[var(--border-primary)] rounded-lg shadow-xl overflow-hidden max-h-60 overflow-y-auto custom-scrollable">
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-left px-3.5 py-2.5 text-sm transition-colors flex items-center justify-between font-poppins ${value === opt ? 'bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 font-semibold' : 'text-[var(--text-secondary)] hover:bg-[var(--component-background-hover)]'}`}
            >
              {opt}
  ...
  <div className={`space-y-6 ${className}`}>
    <div className="pb-3 border-b border-[var(--border-primary)]">
      <div className="flex items-center space-x-3 mb-1">
        ...
        <h2 className="text-xl font-bold text-[var(--text-primary)] font-poppins tracking-tight uppercase">{title}</h2>
      </div>
...
const [stage, setStage] = useState<string>('Validation');
  ...
  const [positions, setPositions] = useState<Array<Omit<Position, 'id' | 'isOpen'> & { tempId: number; newQuestion?: string }>>([]);
  const [nextPositionTempId, setNextPositionTempId] = useState(Date.now());
...
const [teamSize, setTeamSize] = useState('');
  ...
  useEffect(() => {
    ...
    setSolution(idea.buildingNow || '');
    setFounderQuote(idea.founderQuote || '');
    setTags(idea.tags.join(', '));
setTeamSize(idea.teamSize ? String(idea.teamSize) : '');
    ...
    setPositions(idea.positions.map((p, i) => ({ ...p, tempId: i, newQuestion: '' })));
  }, [ideaId, getIdeaById]);

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    if (!title || !tagline || !description || !problem || !solution || !founderQuote || !tags || !location || !imagePreviewUrl || !teamSize) {
  ...
  const inputClasses = "block w-full px-3.5 py-2.5 bg-[var(--background-tertiary)] border border-[var(--border-secondary)] rounded-lg shadow-sm placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-purple-500 sm:text-sm text-[var(--text-primary)] font-poppins font-medium transition-colors duration-200";
  const textAreaClasses = `${inputClasses} min-h-[120px] resize-y`;

  return (
    <div className="max-w-3xl mx-auto pb-20 font-poppins">
      <button onClick={() => navigate(-1)} className="mb-6 inline-flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300 group rounded-full px-5 py-2 bg-[var(--background-tertiary)] border border-[var(--border-primary)] shadow-sm">
        <ChevronLeftIcon className="w-3.5 h-3.5 transition-transform group-hover:-translate-x-1" />
        <span>Back</span>
      </button>

      <PageTitle title="Update Project" description="Refine your vision and keep your team requirements up to date."/>
  ...
  />
        ...
        <FormSection title="Visuals & Links" icon={<PhotoIcon />} subtext="Keep your listing visually compelling.">
            ...
            <FormRow label="Website" htmlFor="websiteUrl" icon={<LinkIcon />} subtext="External link to your landing page.">
                <input type="url" id="websiteUrl" value={websiteUrl} onChange={e => setWebsiteUrl(e.target.value)} className={inputClasses} placeholder="https://... (Optional)"/>
            </FormRow>
        </FormSection>

        <FormSection title="Opportunities" icon={<IdentificationIcon />} subtext="Manage your recruitment needs.">
          {positions.map((pos, index) => (
            <div key={pos.tempId} className="space-y-4 p-5 bg-[var(--component-secondary-background)] border border-[var(--border-primary)] rounded-xl relative">
              <div className="flex justify-between items-center mb-2">
                ...
                {positions.length > 1 && (
                  <button type="button" onClick={() => setPositions(positions.filter(p => p.tempId !== pos.tempId))} className="text-red-500 hover:underline text-xs font-bold uppercase font-poppins">Remove</button>
                )}
              </div>
              <FormRow label="Title" htmlFor={`pos-title-${pos.tempId}`} isRequired subtext="e.g. Lead Designer">
                <input type="text" id={`pos-title-${pos.tempId}`} value={pos.title} onChange={e => {
              ...
              <FormRow label="Commitment" htmlFor={`pos-type-${pos.tempId}`} isRequired subtext="Time or compensation structure.">
                <CustomSelect value={pos.type} onChange={val => {
                  const newPositions = [...positions];
                  const idx = newPositions.findIndex(p => p.tempId === pos.tempId);
              ...
              <FormRow label="Details" htmlFor={`pos-desc-${pos.tempId}`} isRequired subtext="Responsibilities and requirements.">
                <textarea id={`pos-desc-${pos.tempId}`} value={pos.description} onChange={e => {
                  const newPositions = [...positions];
                  const idx = newPositions.findIndex(p => p.tempId === pos.tempId);
                  newPositions[idx].description = e.target.value;
                  setPositions(newPositions);
                }} rows={3} required className={inputClasses}/>
              </FormRow>
                  ...
                  <div className="space-y-2 mb-3">
                      {pos.questions?.map((q, qIndex) => (
                          <div key={qIndex} className="flex items-center gap-2">
                              <div className="flex-grow text-xs font-medium text-[var(--text-primary)] bg-[var(--background-primary)] px-3 py-2 rounded-lg border border-[var(--border-primary)]">{q}</div>
                              <button type="button" onClick={() => {
                                  const newPositions = [...positions];
                                  const idx = newPositions.findIndex(p => p.tempId === pos.tempId);
                                  if (newPositions[idx].questions) {
                                      newPositions[idx].questions = newPositions[idx].questions!.filter((_, i) => i !== qIndex);
                                      setPositions(newPositions);
                                  }
                              }} className="text-red-500 hover:text-red-700 p-1"><XMarkIcon className="w-4 h-4"/></button>
                          </div>
                      ))}
                  </div>
                  <div className="flex gap-2">
                      <input 
                        ...
                        onKeyDown={e => {
                            if (e.key === 'Enter') {
                                e.preventDefault();
                                if (pos.newQuestion?.trim()) {
                                    const newPositions = [...positions];
                                    const idx = newPositions.findIndex(p => p.tempId === pos.tempId);
                                    newPositions[idx].questions = [...(newPositions[idx].questions || []), pos.newQuestion.trim()];
                      ...
                      }} className="bg-[var(--background-tertiary)] hover:bg-[var(--component-background-hover)] text-[var(--text-primary)] text-xs font-bold px-4 py-2 rounded-lg border border-[var(--border-primary)] transition-colors">Add</button>
                  </div>
              </div>
            </div>
          ))}
          <button type="button" onClick={() => {
              setPositions([...positions, { tempId: nextPositionTempId, title: '', description: '', type: 'Paid', skills: [], salaryRange: '', equityOffered: '', questions: [], newQuestion: '' }]);
              setNextPositionTempId(nextPositionTempId + 1);
          }} className="text-purple-600 hover:underline text-[10px] font-bold uppercase tracking-widest font-poppins">+ Add Another Opening</button>
        </FormSection>

        <div className="pt-8 flex justify-end space-x-4 border-t border-[var(--border-primary)]">
           <Link to="/my-projects" className="px-8 py-2.5 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] bg-[var(--background-tertiary)] rounded-full border border-[var(--border-secondary)] transition-all font-poppins text-center">Cancel</Link>
          <button type="submit" disabled={isLoading} className="button-gradient px-10 py-2.5 text-[10px] font-bold uppercase tracking-widest text-white rounded-full transition-all shadow-md transform hover:scale-105 active:scale-95 disabled:opacity-50 font-poppins">
