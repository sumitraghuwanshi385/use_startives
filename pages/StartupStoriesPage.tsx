...
const CustomDropdown: React.FC<CustomDropdownProps> = ({ label, value, onChange, options, icon }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button type="button" onClick={() => setIsOpen(!isOpen)} className={`flex items-center justify-center gap-1 px-2 py-1 text-[10px] font-bold rounded-full transition-all duration-300 border shadow-none ${isOpen ? 'bg-white dark:bg-neutral-800 border-purple-500' : 'bg-gray-100 dark:bg-neutral-800 border-transparent hover:border-purple-500/50'} text-[var(--text-primary)] focus:outline-none flex-1 font-poppins`}>
...
options={modelOptions.map(m => ({value: m, label: m}))} onChange={setActivePricing} icon={<GrowthIcon className="w-4 h-4" />} /><CustomDropdown label="All locations" value={activeLocation} options={['All', ...COUNTRIES.map(c => c.name)].map(l => ({value: l, label: l}))} onChange={setActiveLocation} icon={<MapPinIcon className="w-4 h-4" />} /></div></div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow">{filteredIdeas.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{filteredIdeas.map((idea:any) => <ExchangeCard key={idea.id} idea={idea} />)}</div> : <div className="text-center py-20 bg-[var(--component-background)] rounded-3xl border-2 border-dashed border-[var(--border-primary)] shadow-none"><p className="text-[var(--text-muted)] font-black uppercase tracking-widest text-sm font-poppins">No assets found matching filters</p><button onClick={() => { setActiveCategory('All'); setActivePricing('All'); setActiveLocation('All'); setSearchTerm(''); }} className="mt-4 text-purple-600 font-bold uppercase text-[10px] hover:underline font-poppins">Clear all filters</button></div>}</div>
    </div>
  );
};
