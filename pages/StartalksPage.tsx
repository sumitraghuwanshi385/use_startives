...
const shuffleStartalks = <T,>(items: T[]): T[] => {
  const result = [...items];
...
const StartalksPage: React.FC = () => {
  ...
  const shuffledSessionRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (!userId) {
      shuffledSessionRef.current = null;
      setShuffledFeed([]);
      return;
    }

    if (!startalks || startalks.length === 0) return;

    const sessionId = String(userId);

    if (shuffledSessionRef.current === sessionId) return;

    const newShuffledFeed = shuffleStartalks(startalks);
    setShuffledFeed(newShuffledFeed);
    shuffledSessionRef.current = sessionId;
  }, [currentUser, startalks]);

  const handlePost = async () => {
    const trimmedContent = newTalkContent.trim();

    if (!trimmedContent || isPosting || isImageUploading) return;
    if (trimmedContent.length > MAX_STARTALK_LENGTH) return;

    try {
      setIsPosting(true);
      await addStartalk(trimmedContent, imagePreview || undefined);
      setNewTalkContent('');
  ...
  const filteredTalks = useMemo(() => {
    ...
    return shuffledFeed.filter((shuffledTalk) =>
      startalks.some((currentTalk) => String(currentTalk.id) === String(shuffledTalk.id))
    );
  }, [activeFilter, startalks, shuffledFeed]);

  const characterCount = newTalkContent.length;
  const characterProgress = Math.min(100, (characterCount / MAX_STARTALK_LENGTH) * 100);
  const isAtCharacterLimit = characterCount >= MAX_STARTALK_LENGTH;
    ...
    <div className="bg-[var(--background-secondary)] min-h-screen font-poppins">
      <div className="w-full px-4 md:px-8 lg:px-16 xl:px-24 pt-2 pb-8">
        ...
        <div className="text-left mb-6">
          ...
          <p className="mt-1 text-sm md:text-lg text-[var(--text-secondary)] leading-relaxed font-medium font-['Poppins']">
          ...
          <div className="bg-[var(--component-background)] rounded-3xl border border-[var(--border-primary)] p-6 md:p-8 mb-6 shadow-none relative overflow-hidden text-left font-poppins">
            ...
            <div className="relative z-10">
              <textarea
                ...
                onChange={(e) =>
                  setNewTalkContent(e.target.value.slice(0, MAX_STARTALK_LENGTH))
                }
                placeholder="What's happening in your venture?"
                className="w-full bg-transparent border-none focus:ring-0 focus:outline-none text-[var(--text-primary)] font-medium text-base md:text-lg resize-none min-h-[120px] md:min-h-[150px] placeholder:text-[var(--text-muted)] placeholder:font-['Poppins'] font-['Poppins']"
                maxLength={MAX_STARTALK_LENGTH}
              />
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-[var(--border-primary)]">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative w-9 h-9 flex items-center justify-center">
                    <svg viewBox="0 0 36 36" className="absolute w-9 h-9 -rotate-90">
                      <defs>
                      ...
                      <circle
                        ...
                        r="15.915"
                        strokeWidth="3"
                        stroke={isAtCharacterLimit ? '#ef4444' : 'url(#talkGradient)'}
                        fill="none"
                        strokeDasharray="100"
                        strokeDashoffset={100 - characterProgress}
                        strokeLinecap="round"
                    ...
                    <span
                      className={`text-[9px] font-bold ${
                        isAtCharacterLimit ? 'text-red-500' : 'text-[var(--text-primary)]'
                      }`}
                    >
                      {characterCount}
                    </span>
                ...
                <button
                  ...
                  disabled={
                    !newTalkContent.trim() ||
                    characterCount > MAX_STARTALK_LENGTH ||
                    isPosting ||
                    isImageUploading
                  }
                  className="button-gradient px-6 py-2 rounded-full text-white text-[10px] font-black uppercase tracking-widest shadow-none hover:scale-105 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed font-poppins"
                >
