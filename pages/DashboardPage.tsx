...
import { IdeaStarIcon, ChevronRightIcon, SparklesIcon, ShoppingBagIcon, TwitterXIcon, ChatBubbleLeftRightIcon, GlobeAltIcon, ChatBubbleBottomCenterTextIcon, XMarkIcon, UserCircleIcon, ChatBubbleLeftRightIcon, SparklesIcon, InstagramIcon, PaperAirplaneIcon, GlobeModernIcon } from '../constants';
...
const totalVentures = myProjects.length + myAssets.length;
  ...
  const existingIdeaIds = startupIdeas
    .map((idea) => getSafeId(idea?.id))
    .filter(Boolean);

  // VALID SENT
  const validSent = sentApplications.filter((app) => {
    const ideaId = getSafeId(app?.ideaId);
    const applicantId = getSafeId(app?.applicantId);

    return (
      applicantId === currentUser.id &&
      ideaId &&
      existingIdeaIds.includes(ideaId)
    );
  }).length;
  ...
  const validReceived = receivedApplications.filter((app) => {
    const ideaId = getSafeId(app?.ideaId);
    return ideaId && myProjectIds.includes(ideaId);
  }).length;
...
return validIdeas.length;
  ...
  const stats = [
    { title: 'Products', value: totalVentures, icon: '🚀', subtext: 'Active Products', isPrimary: true, linkTo: '/my-projects', animationDelay: '0.1s' },
    ...
    { title: 'Applications', value: totalApplicationsCount, icon: '📨', subtext: 'Track opportunities', linkTo: '/my-applications', animationDelay: '0.3s' },
     { title: 'Whitelist', value: totalSavedCount, icon: '⭐', subtext: 'Saved Products', linkTo: '/saved-projects', animationDelay: '0.4s' },
  ];
