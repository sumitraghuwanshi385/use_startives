...
const Header: React.FC = () => {
  ...
  const handleSearchClick = () => {
    ...
    setTimeout(() => setIsSearchAnimating(false), 300);
    navigate('/search');
  };
  ...
  const profileDropdownLinks = [
    ...
    { name: 'Discover Ideas', path: '/projects' },
    { name: 'Startup Stories', path: '/stories' },
    ...
    <header className="sticky top-0 z-40 w-full bg-[var(--background-primary)] border-b border-[var(--border-primary)]">
      <div className="container mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3">
        ...
        <div className="flex items-center space-x-0.5 sm:space-x-1">
          {currentUser ? (
            <>
              <button onClick={handleSearchClick} className={commonIconButtonClasses} aria-label="Open search">
                <MagnifyingGlassIcon className={`w-6 h-6 ${isSearchAnimating ? 'animate-icon-click' : ''}`} />
              </button>
